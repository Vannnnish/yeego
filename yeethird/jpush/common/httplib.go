...
// HttpRequest provides more useful methods for requesting one url than yeehttp.Request.
...
// SetTimeout sets connect yeetime out and read-write yeetime out for Request.
...
// example:
//
//	func(req *yeehttp.Request) (*url.URL, error) {
...
func (b *HttpRequest) getResponse() (*http.Response, error) {
	...
	if trans == nil {
		...
		// if b.transport is *yeehttp.Transport then set the settings.
...
// ToFile saves the body data in response to one yeefile.
// it calls Response inner.
...
// TimeoutDialer returns functions of connection dialer with timeout settings for yeehttp.Transport Dial field.
