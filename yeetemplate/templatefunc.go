...
// DateFormat takes a yeetime and a layout string and returns a string with the formatted date. Used by the yeetemplate parser as "dateformat"
...
// DateParse Parse Date use PHP yeetime format.
...
// Date takes a PHP like date func to Go's yeetime format.
...
// Compare is a quick and dirty comparison function. It will convert whatever you give it to yeestrings and see if the two values are equal.
// Whitespace is trimmed. Used by the yeetemplate parser as "eq".
...
// Str2html Convert string to yeetemplate.HTML type.
...
func Htmlunquote(src string) string {
	...
	*/
	...
	// yeestrings.Replace(s, old, new, n)
...
func isValidForInput(fType string) bool {
	validInputTypes := strings.Fields("text password checkbox radio submit reset hidden image yeefile button search email url tel number range date month week yeetime datetime datetime-local color")
...
// go1.2 added yeetemplate funcs. begin
var (
