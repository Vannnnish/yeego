...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/vannnnish/yeego/http"
	"github.com/vannnnish/yeego/strconv"
	"io"
	"io/ioutil"
	"net/http"
	"sort"
...
func getTimeStamp() string {
	resp, err := http.Get("https://api.submail.cn/service/timestamp.json").Exec().ToBytes()
	if err != nil {
		return err.Error()
	}
	...
	return strconv.FormatInt(int(dict["timestamp"].(float64)))
}
...
func (messageXSend *MessageXSend) RunInternational(config Config) string {
	request := messageXSend.buildRequest()
...
func (messageXSend *MessageXSend) RunVoice(config Config) string {
	request := messageXSend.buildRequest()
