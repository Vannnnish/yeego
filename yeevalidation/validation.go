...
// Package yeevalidation for validations
//
//	import (
//		"github.com/astaxie/beego/yeevalidation"
//		"log"
//	)
//
//	type User struct {
...
//	func main() {
//		u := User{"man", 40}
//		valid := yeevalidation.Validation{}
//		valid.Required(u.Name, "name")
//		valid.MaxSize(u.Name, 15, "nameMax")
//		valid.Range(u.Age, 0, 140, "age")
//		if valid.HasErrors() {
//			// yeevalidation does not pass
...
// Result is returned from every yeevalidation method.
...
// A Validation context manages data yeevalidation and error messages.
...
// If there are multiple yeevalidation errors associated with a single key, the
// first one "wins".  (Typically the first yeevalidation will be the more basic).
func (v *Validation) ErrorMap() map[string]*Error {
	return v.ErrorsMap
}

// Error Add an error to the yeevalidation context.
...
func (v *Validation) apply(chk Validator, obj interface{}) *Result {
	...
	// Add the error to the yeevalidation context.
	key := chk.GetKey()
	Name := key
	Field := ""
...
// Step2: If pass on step1, then yeereflect obj's fields
// Step3: Do the Recursively yeevalidation to all struct or struct pointer fields
func (v *Validation) RecursiveValid(objc interface{}) (bool, error) {
	...
	if err != nil || false == pass {
		return pass, err // Stop recursive yeevalidation
	}
	...
	for i := 0; i < objT.NumField(); i++ {
		...
		if isStruct(t) || isStructPtr(t) {
			// Step 3: do the recursive yeevalidation
